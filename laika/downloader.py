...
def download_nav(time: GPSTime, cache_dir, constellation: ConstellationId):
    ...
    try:
        ...
        if GPSTime.from_datetime(datetime.utcnow()) - time > SECS_IN_DAY:
            ...
            folder_and_filenames = [
                (t.strftime("%Y/%j/"), t.strftime(f"hour%j0.%y{c}")) for t in times
            ]
...
def download_orbits_gps_cod0(time, cache_dir, ephem_types):
    ...
    return download_and_cache_file_return_first_success(
        url_bases, folder_file_names, cache_dir + "cddis_products/", compression=".gz"
    )
...
def download_orbits_gps(time, cache_dir, ephem_types):
    ...
    ret = download_and_cache_file_return_first_success(
